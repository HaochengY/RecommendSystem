...
from models.utils.Layers import Embedding_layer
from models.utils.LargeScalePreprocess import DataRecorder


class BaseModel(nn.Module):
    def __init__(self, model_name, dataset_name="Company",batch_size=256,
                 patience=5, embedding_dim=12, num_epochs=30, learning_rate=0.01,
                 criterion_type="BCE", optimizer_type="AdamW",weight_decay=0, 
                record=True, down_sample=5):
    ...
    def train_model(self):
        ...
        if self.record:
            ...
            print("=================================================================")
            print("模型参数:")
            print(f"负采样比例:{self.dataRecorder.down_sample}%")
        ...
        for epoch in range(self.num_epochs):
            ...
            with tqdm(total=len(self.dataRecorder.train_loader),desc=f"Epoch {epoch+1}/{self.num_epochs}", unit="batch") as pbar:
                for i, (inputs, labels, _) in enumerate(self.dataRecorder.train_loader):
            ...
            with torch.no_grad():  # 在评估阶段不计算梯度
                with tqdm(total=len(self.dataRecorder.val_loader),desc=f"Validation {epoch+1}/{self.num_epochs}", unit="batch") as pbar:
                    for inputs, labels, _ in self.dataRecorder.val_loader:
                        inputs = inputs.to(self.device)
                        labels = labels.to(self.device)
                        outputs = self.forward(inputs)
    ...
    def show_evaluation_results(self):
        ...
        train_auc_score, train_gauc = self.evaluate_model(self.dataRecorder.train_loader)
        test_auc_score, test_gauc = self.evaluate_model(self.dataRecorder.test_loader)
        ...
        print(f"                      {self.model_name} Model Result                            ")
        print("=================================================================")
        print(f'Train AUC: {train_auc_score:.4f}, Train GAUC: {train_gauc:.4f}')
        print(f'Test AUC: {test_auc_score:.4f}, Train GAUC: {test_gauc:.4f}')
        ...
        root_dir = f'/home/yanghc03/python_from_github/RecommendSystem/result/{self.model_name}/{today_date}/{exact_time}'
        ...
        if self.record:
            ...
            log_messages.extend([
                ...
                "=================================================================",
                f'Train AUC: {train_auc_score:.4f}, Train GAUC: {train_gauc:.4f}',
                f'Test AUC: {test_auc_score:.4f}, Test GAUC: {test_gauc:.4f}',
    ...
    def evaluate_model(self, data_loader):
        ...
        with torch.no_grad():  # 在评估阶段不计算梯度
            with tqdm(total=len(data_loader),desc="Evaluating", unit="batch") as pbar:
                ...
                for inputs, labels, sponsor_ids in data_loader:
                    inputs = inputs.to(self.device)
                    labels = labels.to(self.device)
                    outputs = self.forward(inputs)
                    outputs = torch.sigmoid(outputs)
