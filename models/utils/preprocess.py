...
class DataRecorder:
    def __init__(self, data_path="../../dataset/train_data.csv", embedding_dim=12, batch_size=32):
        ...
        for column in self.feature_name_list:
            ...
            self.embeddings_to_1_dim_dict[column] = nn.Embedding(num_embeddings=len(le.classes_),
                                                        embedding_dim=1)
            # 存储Embedding层
        ...
        # f"Input Tensor Size: ({self.batch_size}, {self.input_dim}),
        # where {self.input_dim} = {self.feature_num} * {self.embedding_dim}")
        print("============================================")
