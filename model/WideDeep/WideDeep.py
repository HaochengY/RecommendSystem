...
class WideDeep(BaseModel):
    def __init__(self,learning_rate, weight_decay,embedding_dim,
        ...
        super(WideDeep, self).__init__(model_name="WideDeep",
        ...
        self.wide = LinearRegression_layer(self.dataRecorder).to(self.device)
    ...
    for down_sample in [5]:
        for embedding_dim in [12, 16]:
            for hidden_dim_list in [[512, 256]]:
                model = WideDeep(learning_rate=0.01, weight_decay=0.3,embedding_dim = embedding_dim, down_sample=down_sample, 
